...
def gpu_worker(gpu_id, task_queue, result_queue, args):
    ...
    model = YOLO(args.model_path, verbose=False)  # 修复模型版本
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_path', type=str, help='Path to the model file', required=True)
    parser.add_argument('--dataset_path', type=str, help='Name of the model', required=True)
    parser.add_argument('--output_path', type=str, help='Name of the model', required=False)
    ...
    for chunk in pd.read_csv(args.csv_path, usecols=["path"], chunksize=chunksize):
        ...
        video_paths = [os.path.join(args.dataset_path, path) for path in video_paths]
    ...
    # ====== 合并结果 ======
    # 保存结果的CSV文件路径
    output_path = args.output_path if args.output_path else os.path.splitext(args.csv_path)[0] + "_yolo.csv"
